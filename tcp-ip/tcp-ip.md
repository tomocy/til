...
アプリケーション層はトランスポート層による通信可能を前提として新たなプロトコルを定める必要がある。そうでなければ例えばまずサーバからの情報を待つクライアントとまずクライアントの情報を待つサーバではデッドロックが発生してしまう。トランスポート層の上位プロトコルとしてはHTTPやSMTPがあげられる。また上位プロトコルは通信によりトランスポート層から提供されるデータをどう扱うかについても決めておく必要がある。

バイトオーダとはバイナリデータの格納方法である。データの桁が大きい方から格納していく方法をビッグエンディアン、データの桁が小さい方から格納していく方法をリトルエンディアンと呼ぶ。TCP/IPではヘッダ情報に含まれるバイナリデータはビッグエンディアンで格納している。そのためビッグエンディアンはネットワークバイトオーダとも呼ばれる。ちなみにバイトオーダはCPUによっても異なる。そのためバイトオーダのどちらの方式を採用しても、その処理が行われるCPUごとに方式の変換を行う必要がある。

CRLFとはCRとLFから成る。CRとはCarriage returnのことで、カーソルを同行の先頭へと移動させる動作を表す制御コードである。行頭復帰とも言う。一方LFはLine Feedのことで、カーソルを次行の同じ位置に移動させる動作を表す制御コードである。改行ともいう。ディスプレイが普及すると、行頭復帰と改行がまとめて行われることが普通となった。その際にこの動作をCR、LFのどちらかで表すか、続けて表すかということが問題となった。この動作をWindows等ではCRLFと続けて表したのに対し、macOSではLFで表した。そのため異なる表象の仕方を採用しているシステム間では、相互の変換が必要となる。

ハイパーテキストとは参照するテキストをハイパーリンクという形式で表すテキストのことである。ハイパーテキストの表現にはHTML(HyperText Markup Language)やXML(Extensible Markup Language)等が使用される。

HTTPのレスポンスのヘッダにはステータスコード(Status Code)とリーゾンフレーズ(Reason phrace)が含まれる。ステータスコードとは200等のことで、リーゾンフレーズは200に対するOKのことである。

CGIとはCommon Gateway Interfaceのことで、サーバのプログラムから外部のプログラムを呼び出すための機能である。サーバは原来クライアントからの要求に応じて、すでに用意してあるファイルを返答することが役割であった。しかし時代が過ぎクライアントによって返答するファイル、あるいはその中身を変更する需要が出てきた。そこでサーバから外部のプログラムを呼びだす機能が必要となり、CGIが考案された。CGIはサーバから受けっとったクライアントのリクエストを元にファイルを動的に変更する。そしてサーバはそのファイルをクライアントへ返答する。